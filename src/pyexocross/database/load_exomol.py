...
def _extend_exomol_pf(pf_path, T_grid, Q_grid, T_needed_max):
    """
    Extend an ExoMol partition-function table up to ``T_needed_max`` by holding
    Q(T) constant at its last tabulated value, appending to ``pf_path`` in place.

    ExoMol's own ``.pf`` files (downloaded from the archive, or generated by an
    earlier PyExoCross run with a smaller ``Tmax``) cover a fixed temperature
    range, and a run's requested ``Temperatures`` list can exceed it. Rather than
    failing, the table is extended with a constant-value placeholder -- this is
    NOT a physical extrapolation (the true partition function keeps increasing
    with T); it only prevents a hard failure so the run can proceed with an
    explicitly-flagged approximation above the real tabulated range.
    ...
    true original data, not a previously-extended version).
    ...
    print(
        f"Warning: ExoMol partition function file '{pf_path}' only tabulates Q(T) "
...
def read_exomol_pf(read_path, data_info, T_list):
    ...
    if T_arr.max() > T_grid[-1]:
        T_grid, Q_grid = _extend_exomol_pf(pf_filename, T_grid, Q_grid, T_arr.max())
