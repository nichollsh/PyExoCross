...
def _extend_hitran_pf(pf_path, T_grid, Q_grid, T_needed_max):
    """
    Extend a HITRAN partition-function table up to ``T_needed_max`` by holding
    Q(T) constant at its last tabulated value, appending to ``pf_path`` in place.

    HITRAN's own downloaded Q(T) tables (``download_hitran.py``) cover different
    temperature ranges per species/isotopologue (e.g. this machine's local data has
    O2 up to ~4645 K and CO up to ~9000 K, but O3 only up to 1000 K), and a run's
    requested ``Temperatures`` list can exceed whatever range a given species has.
    Rather than failing, the table is extended with a constant-value placeholder --
    this is NOT a physical extrapolation (the true partition function keeps
    increasing with T); it only prevents a hard failure so the run can proceed with
    an explicitly-flagged approximation above the real tabulated range.

    The original file is backed up once, to ``pf_path + '.bak'``, before the first
    extension -- if that backup already exists, it is left untouched (it holds the
    true original downloaded data, not a previously-extended version).

    Returns the extended ``(T_grid, Q_grid)`` arrays.
    """
    backup_path = pf_path + '.bak'
    if not os.path.exists(backup_path):
        shutil.copy2(pf_path, backup_path)

    old_max_T = float(T_grid[-1])
    clamp_Q = float(Q_grid[-1])
    new_max_T = int(np.ceil(T_needed_max))
    extra_T = np.arange(int(old_max_T) + 1, new_max_T + 1)

    print(
        f"Warning: HITRAN partition function file '{pf_path}' only tabulates Q(T) "
        f"up to {old_max_T:g} K, but this run requests temperatures up to "
        f"{T_needed_max:g} K. Extending the file in place up to {new_max_T:g} K by "
        f"holding Q(T) constant at its {old_max_T:g} K value ({clamp_Q:g}) -- this "
        f"is a placeholder, NOT a physical extrapolation (the true partition "
        f"function continues to increase with T). Cross sections computed above "
        f"{old_max_T:g} K using this extension are approximate and should be "
        f"replaced with real reference data before scientific use. The original "
        f"file was backed up to '{backup_path}'."
    )
    ...
    with open(pf_path, 'a') as fh:
        if last_char != b'\n':
            fh.write('\n')
        for T in extra_T:
            fh.write(f'{int(T):5d}{clamp_Q:20.5f}\n')

    new_T_grid = np.concatenate([T_grid, extra_T.astype(float)])
    new_Q_grid = np.concatenate([Q_grid, np.full(extra_T.shape, clamp_Q)])
    return new_T_grid, new_Q_grid
...
def read_hitran_pf(T_list):
    ...
    if T_arr.max() > T_grid[-1]:
        T_grid, Q_grid = _extend_hitran_pf(pf_path, T_grid, Q_grid, T_arr.max())
